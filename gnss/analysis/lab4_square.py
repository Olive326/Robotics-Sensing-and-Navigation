...
try:
    with open('mag_calibration.pkl', 'rb') as f:
        cal_params = pickle.load(f)
    print("✓ Loaded calibration from mag_calibration.pkl")
    print(f"  X offset: {cal_params['x_offset']:.4f}")
    print(f"  Y offset: {cal_params['y_offset']:.4f}")
except FileNotFoundError:
    print("ERROR: mag_calibration.pkl not found!")
    print("Please run circle data analysis first!")
    exit()
    
# ========== APPLY CALIBRATION (DO NOT RECALIBRATE) ==========
def apply_calibration(mag_x, mag_y, cal_params):
   # Apply existing calibration to new data
    x_centered = mag_x - cal_params['x_offset']
    y_centered = mag_y - cal_params['y_offset']
    
    x_cal = x_centered * (cal_params['avg_scale'] / cal_params['x_scale'])
    y_cal = y_centered * (cal_params['avg_scale'] / cal_params['y_scale'])
    
    return x_cal, y_cal

# Apply circle calibration to square data
mag_x_cal, mag_y_cal = apply_calibration(mag_x, mag_y, cal_params)
